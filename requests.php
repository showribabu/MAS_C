    ...
    if($rr)
    {
        ...
        header("Location: credentials.php?user_id=" . urlencode($uid2));    
                ...
                if(isset($_GET['duser_id']))
                {
                    ...
                    $sql12='insert into requests(r_status,request_to,request_from,group_number,group_type,message,request_id) values("ar","'.$gmid.'","'.$uid3.'","'.$_SESSION['group_number'].'","'.$_SESSION['group_type'].'","You are removed from a group :GM !!!! If any further information contact!!!","'.$request_id.'")';
                    $rr=mysqli_query($con,$sql12);
                    if($rr)
                    {
                        echo"<script>alert('Accept Button Clicked');</script>";
